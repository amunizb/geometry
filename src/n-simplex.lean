...
begin
  ...
  have h1: ∑ (j : ℕ) in finset.range (n + 2), g j = 1,
  {
    ...
    rw h4,
    rw add_zero,
    have x_in_s_kc_implies_x_gt_k: ∀ (x : ℕ), x ∈ s_kc → x > k,
    {
      intros x hx,
      rw hs_kc at hx,
      simp at hx,
    ...
    have j_in_s_kc_implies_g_j_jm1: ∀ (j : ℕ), j ∈ s_kc → g j = face_pnt.val (j-1),
    {
      intros j hj,
      have j_gt_k : j > k := x_in_s_kc_implies_x_gt_k j hj,
      rw hg,
      apply shift_at_gt_n k face_pnt.val j,
      exact j_gt_k,
    },
    set s_km1 := finset.range k with hs_km1,
    have j_in_s_km1_implies_g_j_j: ∀ (j : ℕ), j ∈ s_km1 → g j = face_pnt.val j,
    {
      intros j hj,
      rw hg,
      apply shift_at_lt_n k face_pnt.val j,
      rw hs_km1 at hj,
      simp at hj,
      exact hj,
    },
    have sum_g_j_jm1: ∑ (j : ℕ) in s_kc, g j = ∑ (j : ℕ) in s_kc, face_pnt.val (j-1),
    {
      apply finset.sum_congr,
      refl,
      intros j hj,
      exact j_in_s_kc_implies_g_j_jm1 j hj,
    },
    have sum_g_j_j: ∑ (j : ℕ) in s_km1, g j = ∑ (j : ℕ) in s_km1, face_pnt.val j,
    {
      apply finset.sum_congr,
      refl,
      intros j hj,
      exact j_in_s_km1_implies_g_j_j j hj,
    },
    rw sum_g_j_jm1,
    rw sum_g_j_j,
    set s_km1c := finset.range (n+1) \ s_km1 with hs_km1c,
    have h5: disjoint s_km1 s_km1c,
    {
      rw finset.disjoint_iff_inter_eq_empty,
      rw hs_km1c,
      simp,
    },
    have h6: finset.range (n+1) = s_km1.disj_union s_km1c h5,
    {
      rw finset.disj_union_eq_union,
      rw hs_km1c,
      simp,
    ...
    have h7: ∑ (j : ℕ) in s_kc, face_pnt.val (j-1) = ∑ (j : ℕ) in s_km1c, face_pnt.val j,
    {
      rw hs_kc,
      rw hs_km1c,
      rw hs_k,
